...
@section ('title', trans('labels.backend.quiz.board.management') . ' | ' . trans('labels.backend.quiz.board.edit'))

@section('page-header')
    <h1>
        {{ trans('labels.backend.quiz.board.management') }}
        <small>{{ trans('labels.backend.quiz.board.edit') }}</small>
    </h1>
@endsection
        ...
        <div class="box box-success">
            ...
            <div class="box-body">
                <div class="form-group">
                    ...
                    <div class="col-lg-10">
                        <input class="form-control" maxlength="191" required="required" autofocus="autofocus" placeholder="{{ trans('validation.attributes.backend.quiz.boards.name') }}" name="name" type="text" id="name" value='{{ old('name',$board->name) }}'>
                    </div>
                </div>
                <div class="form-group">
                    <label for="description" class="col-lg-2 control-label">{{ trans('validation.attributes.backend.quiz.boards.description') }}</label>
                    <div class="col-lg-10">
                        <textarea class="form-control" placeholder="{{ trans('validation.attributes.backend.quiz.boards.description') }}" name="description" cols="50" rows="10" id="description">{{ old('description',$board->description) }}</textarea>
                    </div>
                </div>
                <div class="form-group">
                    <label for="location" class="col-lg-2 control-label">{{ trans('validation.attributes.backend.quiz.boards.location') }}</label>
                    <div class="col-lg-10">
                        <textarea class="form-control" placeholder="{{ trans('validation.attributes.backend.quiz.boards.location') }}" name="location" cols="50" rows="10" id="location">{{ old('location',$board->location) }}</textarea>
                    </div>
                </div>
                <div class="form-group">
