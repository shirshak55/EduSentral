...
@section ('title', trans('labels.backend.quiz.set.management') . ' | ' . trans('labels.backend.quiz.set.edit'))

@section('page-header')
    <h1>
        {{ trans('labels.backend.quiz.set.management') }}
        <small>{{ trans('labels.backend.quiz.set.edit') }}</small>
    </h1>
@endsection
    ...
    <form method="POST" action="{{ route('admin.quiz.sets.update',$set->id)}}" accept-charset="UTF-8" class="form-horizontal" role="form" id="edit-set">
        ...
        <div class="box box-success">
            <div class="box-header with-border">
                <h3 class="box-title">{{ trans('labels.backend.quiz.set.edit') }}</h3>
            ...
            <div class="box-body">
                <div class="form-group">
                    ...
                    <div class="col-lg-10">
                        <input class="form-control" maxlength="191" required="required" placeholder="{{ trans('validation.attributes.backend.quiz.set.name') }}" name="name" type="text" id="name" value='{{ old('name',$set->name) }}'>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-lg-10">
                        <select class="form-control" id="year" name="year" placeholder='Select Year when this question was published'>
                            ...
                            @for($year=1950;$year<=date("Y");$year++)
                                <option value="{{ $year }}"  @if($year == old('year',$set->year)) <?php $yearFlag=true?> selected='selected' @elseif($year == 2017 && $yearFlag==false) selected='selected' @endif>{{ $year }}</option>
                            @endfor
                        </select>
                    </div>
                </div>
                    ...
                    <div class="col-lg-10">
                        <select class="form-control" maxlength="191" required="required" id="board" name="board" placeholder='Select Board:'>
                            ...
                            @foreach($boards as $board)
                                <option value="{{ $board->id }}" {{ $board->id == old('board',$set_board->id)? 'selected=selected': '' }} >{{ $board->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="col-lg-10">
                        <select class="form-control" maxlength="191" required="required" id="rule" name="rule">
                            ...
                            @foreach($rules as $rule)
                                <option value="{{ $rule->id }}" {{ $rule->id == old('rule',$set_rule->id)? 'selected=selected' : '' }} >{{ $rule->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
        ...
        <div id='question_wrapper'>
            ...
            <div class="questions" id='questions'>
                ...
                @foreach($questions as $question)
                    <div class="question" data-question='{{ $question->sort }}'>
                        <div class="box box-alert">
                            <div class="box-body">
                                 <div class="question_wrapper">
                                    <div class='question-details form-group row'>
                                        <div class="col-md-4" >
                                            <select name="question[{{ $question->sort }}][difficulty]" class='form-control question_difficulty'>
                                                @foreach(['very easy','easy','moderate','hard','very hard'] as $difficulty)
                                                    <option value="{{ $difficulty}}" {{ $question->difficulty == $difficulty? 'selected=selected': ''  }}>{{ ucwords($difficulty)}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-4" ><input type="text" class='form-control question_marks' placeholder='Enter marks' name='question[{{ $question->sort }}][marks]' value='{{ $question->marks }}'></div>
                                        <div class="col-md-4" ><input type="text" class='form-control question_time' placeholder='Enter time(second)' name='question[{{ $question->sort }}][time]' value='{{ $question->time }}'></div>
                                    </div>
                                    <div class='form-group'>
                                        <label for="question_{{ $question->sort }}" class='col-lg-2 control-label question_content_label'>{{ trans('validation.attributes.backend.quiz.set.question')}} <span class='question_number_text'>{{ $question->sort }}</span>.</label>
                                        <div class="col-lg-10">
                                            <textarea  class='form-control question_content'   name='question[{{ $question->sort }}][content]' id='question_{{ $question->sort }}' rows='2'>{{ $question->content }}</textarea>
                                        </div>
                                    </div>
                                    <div class="answers">
                                        @foreach($question->answers as $answer)
                                            <div class='form-group answer' data-question='{{$question->sort}}' data-answer='{{ $answer->sort }}'>
                                                <label for="answer_{{$question->sort}}_{{ $answer->sort }}" class='col-lg-2 control-label answer_content_label'>{{ trans('validation.attributes.backend.quiz.set.answer')}} <span class='answer_number'>{{ $answer->sort }}</span>.</label>
                                                <div class="col-lg-10 ">
                                                    <div class="input-group">

                                                    <textarea  class='form-control answer_content'   name='answer[{{$question->sort}}][{{ $answer->sort }}]' id='answer_{{ $question->sort }}_{{ $answer->sort }}' rows='1'>{{ $answer->content }}</textarea>
                                                    <span class="input-group-addon"><input  type='checkbox' class='correct_answer_checkbox' name='correct_answer[{{ $question->sort }}][{{ $answer->sort }}]' {{  $question->correct_answers->pluck('answer_id')->contains($answer->id)? 'checked=checked': '' }}></span>
